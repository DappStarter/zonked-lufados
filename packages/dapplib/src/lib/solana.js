...
class Solana {
    ...
    async getAccountInfo(publicKey) {
        return await this.connection.getAccountInfo(Solana.getAccount(publicKey));
    }

    async getAccountBalance(publicKey) {
        return await this.connection.getBalance(Solana.getAccount(publicKey));
    }

    async airDrop(account, lamports) {
        await this.connection.requestAirdrop(Solana.getAccount(account), lamports);
    }
    ...
    async createSystemAccount() {
        ...
        await self.airDrop(Solana.getAccount(account.publicKey), lamports);
        return account;
    }
    ...
    async createAccount(options) {
        ...
        for (;;) {
            ...
            if (lamports == (await self.getAccountBalance(Solana.getAccount(account.publicKey)))) {
    ...
    async deployProgram(program){
        ...
        let deployAccounts = {
            "payer": {
                ...
                privateKey: binary_to_base58(payerAccount.secretKey),
        ...
        for(let l=0; l<dataLayouts.length; l++) {
            ...
            deployAccounts[dataLayouts[l].name] = {
                            ...
                            privateKey: binary_to_base58(stateAccount.secretKey),
                            lamports
            }          
        }
    ...
    async submitTransaction(options) {
        ...
        // Create all the state accounts
        let transactionAccounts = [ options.payer ];
        let transaction = new Transaction();

        transaction.add(
            new TransactionInstruction({
                keys: options.keys,
                programId: Solana.getAccount(options.programId),
                data: options.data
              })
        );
        debugger;
